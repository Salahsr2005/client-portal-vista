...
import {
  ...
  type DestinationConsultationPreferences,
...
export const useDestinationConsultation = () => {
  ...
  const findMatches = async (preferences: DestinationConsultationPreferences): Promise<MatchedDestination[]> => {
    ...
    const matches = DestinationMatchingService.findMatchingDestinations(destinations, preferences, 10)
  ...
  const completeConsultation = async () => {
    if (
      ...
      !consultationData.intakePeriod ||
      !consultationData.academicLevel
    ) {
