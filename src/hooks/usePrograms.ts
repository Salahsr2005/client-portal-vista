...
export const usePrograms = (params: ProgramsQueryParams = {}) => {
  const { user } = useAuth()
  const { page = 1, limit = 10, preferences = null, calculateMatchScores = false } = params
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ProgramsData> => {
      try {
        // Build and execute query using the safer filter service method
        let query =
          params.field && /[,'"]/g.test(params.field)
            ? ProgramFilterService.buildQueryWithSafeFieldFilter(params)
            : ProgramFilterService.buildQuery(params)
        ...
        const sortedPrograms = ProgramDataProcessor.sortPrograms(processedPrograms, calculateMatchScores, preferences)
