...
const createAndDownloadPDF = (htmlContent: string, filename: string) => {
  try {
    ...
    const fullHtmlContent = `
      ...
      <html>
        ...
        <body>
          ${htmlContent.replace(/<style>.*?<\/style>/s, '')}
    ...
    // Focus and trigger print dialog automatically
    printWindow.focus();
    setTimeout(() => {
      printWindow.print();
    }, 500);
...
export const generateDestinationPDF = async (destination: Destination): Promise<void> => {
  try {
    ...
    const filename = `${destination.name.replace(/[^a-zA-Z0-9]/g, '_')}_Destination_Brochure`;
    createAndDownloadPDF(htmlContent, filename);

  } catch (error) {
    console.error('Error generating destination PDF:', error);
    throw error;
  }
};
...
export const generateProgramPDF = async (program: any): Promise<void> => {
  try {
    ...
    const filename = `${program.name.replace(/[^a-zA-Z0-9]/g, '_')}_Program_Brochure`;
    createAndDownloadPDF(htmlContent, filename);
