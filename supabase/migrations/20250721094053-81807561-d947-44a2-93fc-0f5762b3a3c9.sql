-- Fix send_message function to use correct column names
CREATE OR REPLACE FUNCTION public.send_message(p_chat_id uuid, p_sender_id uuid, p_message_text text, p_sender_type text DEFAULT 'Client'::text)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- Update chat last message info using existing column last_message_at
  UPDATE chats 
  SET 
...
-- Fix mark_messages_as_read function to remove status column reference
